...
function cargarTabla(){
	var html = '';
	var contenedor = $("#Divisiones-tbody");
	if($("#Divisiones-table_wrapper").length != 0){
		$('#Divisiones-table').dataTable().fnDestroy();
	}
	contenedor.empty();
	$.ajax({
		url: 'getDatosTable',
...
function configurarDataTable(){
	...
	$("select[name=Divisiones-table_length]").css({
			'width':'50px', 
		});

	//configurarEditable();

	var paginador = $("#Divisiones-table_paginate");
	...
	$('.edit-able[pk='+id+']').editable({
	    ...
	    success: function(respon, newValue) {
	    	$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: {'pk':id, 'newValue':newValue, entorno: 'Divisiones'},
				success: function(res){
					if(res.mensaje){
...
$("#cerrar").click(function(){
	...
	if(divisiones == 0){
		alertify.error('Debe Ingresar Al Menos Una División');
		return false;
	}
	if(divisiones.length != cantDivisiones){
		alertify.error('Número de Divisiones y Cantidad de Divisiones Ingresadas son Distintas');
		return false;
	}
